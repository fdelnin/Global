...
\paragraph{Caso Induttivo S=$\sum_{i\inI}Pi$} In questo caso $S=P_{1}+P_{2}+P_{3}+...$ e per ipotesi induttiva sappiamo che ogni $Pi$ è stato genetato da un numero finito di "passi di derivazione" e che termina in un numero finito di passi. In questo caso il processo $S$ fa un passo e va in $S'=Pi$ per qualche $i \in I$, quindi $size(S') = size(Pi)$ che per ipotesi è finita dato che il processo termina; $size(S)=max\{size(Pi), i\in I\}$ ma siccome ogni processo $Pi$ è finito per ipotesi, il massimo elemento è comunque finito, e quindi il processo termina in un numero finito di passi dato se si aggiunge un passo (quello della scelta).

Dimsotro per induzione sul numero di processi sommati:
\subparagraph{Caso Base:}
caso base con lunghezza di derivazione 1 è il processo $0$, la cui $size$ è 0.
\subparagraph{Caso Induttivo n+1}
In questo caso per ipotesi induttiva vale per ogni sottoprocesso la lunghezza di derivazione è finita e che $S'=\sum_{n}Pi$ termina in un numero finito di passi.
Aggiungendo il processo $n+1$esimo $S$ ha size finita perchè è $size(S)=max\{size(P0), size(P1)....size(Pn+1)\}$ e sono tutti processi che terminano in un numero finito di passi, per cui la loro $size $ è finita $\implies \; size(S) $ è finita.  
